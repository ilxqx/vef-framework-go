...
// ExportUser is the test model for export tests (uses excel tags)
type ExportUser struct {
	bun.BaseModel `bun:"table:export_user,alias:eu"`
	orm.Model     `bun:"extend" excel:"-"`

	Name   string `json:"name" excel:"姓名,width=20" bun:",notnull"`
	Email  string `json:"email" excel:"邮箱,width=25" bun:",notnull"`
	Age    int    `json:"age" excel:"年龄,width=10" bun:",notnull"`
	Status string `json:"status" excel:"状态,width=10" bun:",notnull,default:'active'"`
}
...
func NewTestUserExportWithOptionsResource() api.Resource {
	return &TestUserExportWithOptionsResource{
		...
		ExportAPI: apis.NewExportAPI[ExportUser, ExportUserSearch]().
			Public().
			ExportOptions(excel.WithSheetName("用户列表")),
	}
}
...
func (suite *ExportTestSuite) TestExportWithFilterApplier() {
	...
	// The filter should only export active users
	// We can't easily verify the content without parsing the Excel file,
...
func (suite *ExportTestSuite) TestExportWithOptions() {
	...
	// The sheet name is set to "用户列表" via ExportOptions
	// We can't easily verify this without parsing the Excel file,
	// but we can verify the file was generated successfully
}
