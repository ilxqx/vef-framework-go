...
// TreeAdapter defines the adapter for building trees from arbitrary data.
type TreeAdapter[T any] struct {
...
// BuildTree converts a flat slice of nodes into a tree structure using the provided adapter.
...
// FindTreeNode finds a node by ID in the tree structure built with TreeAdapter.
func FindTreeNode[T any](roots []T, targetId string, adapter TreeAdapter[T]) (T, bool) {
...
// FindTreeNodePath returns the path from root to the target node for TreeAdapter.
func FindTreeNodePath[T any](roots []T, targetId string, adapter TreeAdapter[T]) ([]T, bool) {
	if targetId == constants.Empty {
		return nil, false
	}
...
func findNodeRecursive[T any](nodes []T, targetKey string, adapter TreeAdapter[T]) (T, bool) {
...
func findNodePathRecursive[T any](node T, targetKey string, currentPath []T, adapter TreeAdapter[T]) ([]T, bool) {
