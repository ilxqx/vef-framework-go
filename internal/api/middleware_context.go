...
func buildContextMiddleware(db orm.Db) fiber.Handler {
	return func(ctx fiber.Ctx) error {
		...
		contextualDb := db.WithNamedArg(orm.PlaceholderKeyOperator, principal.Id)
		contextx.SetDb(ctx, contextualDb)

		request := contextx.APIRequest(ctx)
		logger := contextx.Logger(ctx)
		contextx.SetLogger(ctx, logger.Named(request.Resource+constants.Colon+request.Action+constants.At+request.Version).Named(string(principal.Type)+constants.Colon+principal.Id+constants.At+principal.Name))
