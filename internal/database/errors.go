...
func wrapPingError(dbType constants.DbType, err error) error {
	return newDatabaseError(dbType, "ping", fmt.Errorf("%w: %v", errPingFailed, err), nil)
}

func wrapVersionQueryError(dbType constants.DbType, err error) error {
	return newDatabaseError(dbType, "version_query", fmt.Errorf("%w: %v", errVersionQueryFailed, err), nil)
}
